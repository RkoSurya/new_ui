...
# Initialize the user proxy agent for interaction with user inputs
...
# Initialize the developer agent with updated instructions for processing all files
...
# Initialize the senior developer agent with an updated review-focused prompt
...
# Initialize the group chat with user proxy, developer, and senior developer
...
def list_dir_recursive(directory: str, language: str = "nextjs"):
    ...
    try:
        ...
        # only return files with valid code extensions like .py, .js, .java, etc....
        # print(f'------------{items}----------3')
        items = [
            item
            for item in items
...
# Function to list files recursively based on language
...
def list_dir(
    ...
    try:
        # print("directory coming in list_dir", default_path)
    ...
    if status == 0 and files:
        for file_path in files:
            ...
            while True:
                ...
                if status == 0:
                    # If no issues, file is resolved
                    print(f"Issues resolved in {file_path}. Proceeding to next file.")
                    break

                # Pass unresolved issues to user proxy for resolution
                print(f"Passing {file_path} to user proxy for resolution.")
        ...
        **Process Flow:**
        ...
        Please begin the code review process.
                ...
                # Wait for proxy to process the issues
                print(f"Rechecking {file_path} after proxy resolution...")
...
#             issues = run_npm_command(file_path, "npx eslint . --ext .ts,.tsx --fix")
#             print(f"Issues found in {file_path}: {issues}")

#             if issues:
#                 # Pass issues to the agent for processing
#                 user_proxy.initiate_chat(
#                     manager,
#                     message=f""" 
#         @javascript_developer and @senior_developer, please process the following file: {file_path} 
...
#             - Error handling improvements.
#             - Component structure or styling adjustments.
#         - Provide a code snippet of the updated file in the review for reference. And also save the review feedback using the `store_code_review` function.
#         - End the review with "TERMINATE" to indicate final approval and completion of the process.
...
#                 # Wait for issue resolution and recheck the file
#                 recheck_issues = run_npm_command(file_path, "npx eslint . --ext .ts,.tsx --fix")
#                 if not recheck_issues:
#                     print(f"Issues resolved in {file_path}. Proceeding to next file.")
#                 else:
#                     # Forward unresolved issues to the agent for further action
#                     user_proxy.initiate_chat(
#                         manager,
#                         message=f"""
#         @javascript_developer and @senior_developer, unresolved issues persist in the file: {file_path}
#         **Unresolved Issues:**
#         {recheck_issues}

#         Please review the issues and address them to finalize the file.
#         """
#                     )
#                     print(f"Issues unresolved in {file_path}. Passed to user proxy for review.")
#             else:
...
#         print(f"Error listing files: {files}" if status == 1 else "No files found to process")
    ...
    # review_path = "code_reviews/code_review_log.jsonl"

    # # Store the final review feedback in .md format
    ...
    #     review_entry = json.loads(review)
    #     review_md += f"""
    # ### File Reviewed: {review_entry['file_reviewed']}

    # - **Timestamp**: {review_entry['timestamp']}

    # **Feedback**:

    # {review_entry['feedback']}
    # """

    # with open("code_reviews/code_review_log.md", "w") as f:
    #     f.write(review_md)
